...
class ViewController: UIViewController, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        /* EVALUATION */
        let datasetPath = "./Volumes/Crucial X6/food/" // input path
        let resultPath = "./Volumes/Crucial X6/tests/" // output path
        
        let fileManager = FileManager()
        do{
            let classes = try fileManager.contentsOfDirectory(atPath: datasetPath)

            for cl in classes{
                let srcDir = datasetPath + cl
                let dstDir = resultPath + cl

                if !fileManager.fileExists(atPath: dstDir) {
                    do {
                        try fileManager.createDirectory(atPath: dstDir, withIntermediateDirectories: true, attributes: nil)
                    } catch {
                        print("Couldn't create document directory")
                    }
                }
                
                /* MLKit Object Detection Options */
                ...
                do{
                    let images = try fileManager.contentsOfDirectory(atPath: srcDir)
                    for image in images{
                        let imagePath = srcDir + "/" + image
                        let uiImage = UIImage(contentsOfFile: imagePath)
                        detectObjectsForTest(in: uiImage, options: options, filename: image, destination: dstDir)
                    }
                } catch {
                    print("Error access directory2: \(error)")
                }
            }
        } catch let error as NSError {
            print("Error access directory1: \(error)")
        }
        
//        do{
//            let dataset = try fileManager.contentsOfDirectory(atPath: datasetPath)
//
//            for data in dataset{
//                let srcPath = datasetPath + data
//                print(srcPath)
//
//                let options = ObjectDetectorOptions()
...
//                options.detectorMode = .singleImage
//
//                let uiImage = UIImage(contentsOfFile: srcPath)
//                detectObjectsForTest(in: uiImage, options: options, filename: data, destination: resultPath)
//            }
//        } catch let error as NSError {
...
extension UIImage {
    func save(name: String, destination: String) {
        let path: String = destination
...
extension ViewController {
    ...
    private func detectObjectsForTest(in image: UIImage?, options: CommonObjectDetectorOptions, filename: String, destination: String){
        ...
        detector.process(visionImage) { objects, error in
            ...
            guard error == nil else {
                print(filename, "Object detection failed with error.")
                return
            }
            guard let objects = objects, !objects.isEmpty else {
                print(filename, "On-Device object detector returned no results.")
                let jsonData : [String: Any] = [
                    "MLKit class name": "unknown",
                    "MLKit class confidence": 0.0,
                    "xmin": 0,
                    "ymin": 0,
                    "xmax": 0,
                    "ymax": 0
                ...
                do {
                    let json_filename = filename.replacingOccurrences(of: ".jpg", with: "-mlkit.json")
                    let fileUrl = URL(fileURLWithPath: destination).appendingPathComponent(json_filename)
                    let jsonCreate = try JSONSerialization.data(withJSONObject: jsonData, options: .prettyPrinted)
                    try jsonCreate.write(to: fileUrl, options: [])
                } catch {
...
//                let imageRef:CGImage = newImage.cgImage!.cropping(to: object.frame)!
//                newImage = UIImage(cgImage:imageRef)
//            }
//            newImage.save(name: filename, destination: destination)
            
            for object in objects{
                print(filename + " success")
                var object_label = "unknown"
                var label_confidence:Float = 0.0

                if (object.labels.count != 0){
                    for (index, label) in object.labels.enumerated(){
                        object_label = label.text
                        label_confidence = label.confidence
                    }
                }

                let jsonData : [String: Any] = [
                    "xmin": object.frame.minX,
                    "xmax": object.frame.maxX,
                    "ymin": object.frame.minY,
                    "ymax": object.frame.maxY,
                    "MLKit class name": object_label,
                    "MLKit class confidence": label_confidence
                ] as Dictionary
...
private enum Constants {
    static let images = [
        ...
        "2022-02-17_12=05=25=435%rgb.jpg", "2022-04-21_18=04=22=986%rgb.jpg", "2022-06-03_11=29=33=705%rgb.jpg", "2022-06-16_14=21=57=840%rgb.jpg"
    ]
